...
public class Main20 {
    ...
    /* 10798번 세로 읽기
      ...
      영석이는 칠판에 글자들을 수평으로 일렬로 붙여서 단어를 만든다. 다시 그 아래쪽에 글자들을 붙여서 또 다른 단어를 만든다. 이런 식으로 다섯 개의 단어를 만든다. 아래 그림 1은 영석이가 칠판에 붙여 만든 단어들의 예이다.
    ...
    public static void main(String[]args) throws IOException {
        ...
        sb.append(s1.charAt(0)).append(s2.charAt(0)).append(s3.charAt(0)).append(s4.charAt(0)).append(s5.charAt(0));
        
        // 최소 1글자 에서 최대 15글자 까지
        for (int i=1; i<15; i++) {
            // 1글자는 끝났으니
            if (s1.length() > i) sb.append(s1.charAt(i));
            if (s2.length() > i) sb.append(s2.charAt(i));
            if (s3.length() > i) sb.append(s3.charAt(i));
            if (s4.length() > i) sb.append(s4.charAt(i));
            if (s5.length() > i) sb.append(s5.charAt(i));
        }
